...
- **Step 1 — Prepare a case:** Upload reference documents via the DocuDevs UI or CLI and let the indexing worker push them into the knowledge index. For example:

   ```bash
   # Create or update a case from local files
   python -m docudevs.cli knowledge-base upsert \
     --token "$DOCUDEVS_API_KEY" \
     --api-url "$DOCUDEVS_API_URL" \
     --case-id kb-kitchen-pro \
     --name "Signature Appliances Catalog" \
     --documents ./reference-catalog/*.pdf

   # Trigger indexing if needed
   python -m docudevs.cli knowledge-base index \
     --token "$DOCUDEVS_API_KEY" \
     --api-url "$DOCUDEVS_API_URL" \
     --case-id kb-kitchen-pro
   ```

- **Step 2 — Discover the case ID:** `docudevs cli knowledge-base list` (or the API `GET /knowledge-base`) returns the IDs you can target.
...
async def enrich_products():
  ...
  tool = ToolDescriptor(
    ...
    config=ToolDescriptorConfigType_1.from_dict({
...
python -m docudevs.cli document process \
  --api-url "$DOCUDEVS_API_URL" \
  --token "$DOCUDEVS_API_KEY" \
  --document ./samples/products.csv \
  --mime-type text/csv \
  ...
  --schema @schemas/appliance-catalog.json \
...
- Combine with `docudevs cli knowledge-base list` to fetch available case IDs at runtime.

## Operational notes

- **Environment setup:** If you self-host DocuDevs, configure the knowledge index credentials through your deployment platform so the worker can reach your search cluster. (Hosted DocuDevs environments already include this.)
